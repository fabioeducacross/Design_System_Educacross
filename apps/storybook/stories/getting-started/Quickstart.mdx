...
# Branch master (últimas alterações)
pnpm add github:fabioeducacross/Design_System_Educacross#master
```

> **💡 Importante**: Sempre use tags de versão (`#v0.1.0`) em produção. A branch `master` pode conter mudanças instáveis.
...
> **ℹ️ Status**: Disponível apenas via GitHub no momento. Publicação no npm registry planejada.
...
**5. `src/app/page.tsx`:**
```tsx
import { Button, Card } from "@educacross/ui";
...
**Solução:**
1. Verifique se `@educacross/ui/styles.css` está importado
